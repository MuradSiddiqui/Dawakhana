			...
			<div className="bg-white rounded-3xl p-6 sm:p-8 lg:p-10 shadow-xl border border-gray-100 hover-lift group relative overflow-hidden">
				...
				<div className={`flex flex-wrap items-center gap-2 sm:gap-3 mb-6 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
					...
					{branch.facebookUrl && (
						...
						>
							<Facebook className="h-3 w-3 sm:h-4 sm:w-4 pointer-events-none" /> <span className="hidden sm:inline pointer-events-none">Facebook</span>
						</a>
					)}
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
					<div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-6 sm:p-8 shadow-lg border border-gray-100 relative overflow-hidden">
						...
						<div className={`absolute top-4 w-16 h-16 bg-rose-100 rounded-full blur-2xl opacity-50 ${isRTL ? 'left-4' : 'right-4'}`}></div>
						<h4 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6 relative z-10">{t('branches.openingHours')}</h4>
						<div className="overflow-x-auto relative z-10">
							<table className={`w-full text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
								<thead>
									<tr className="border-b-2 border-rose-200">
										<th scope="col" className={`py-4 text-gray-900 font-bold text-sm ${isRTL ? 'pl-4' : 'pr-4'}`}>{isRTL ? 'دن' : 'Day'}</th>
										<th scope="col" className="py-4 text-gray-900 font-bold text-sm">{isRTL ? 'اوقات' : 'Hours'}</th>
									</tr>
								</thead>
								<tbody>
									{branch.hours.map((h, index) => {
										...
										return (
											<tr key={h.day} className={`border-b border-gray-100 hover:bg-rose-50/50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
												<th scope="row" className={`py-4 font-semibold text-gray-800 text-sm ${isRTL ? 'pl-4' : 'pr-4'}`}>{translatedDay}</th>
												<td className="py-4 text-gray-700 text-sm break-words font-medium" dir="ltr">
													{h.open === 'Closed' ? (
